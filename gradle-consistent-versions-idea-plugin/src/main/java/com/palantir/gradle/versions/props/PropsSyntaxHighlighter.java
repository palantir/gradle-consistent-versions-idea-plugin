...
public class PropsSyntaxHighlighter extends SyntaxHighlighterBase {
    ...
    public final TextAttributesKey[] getTokenHighlights(IElementType tokenType) {
        if (tokenType.equals(PropsTypes.COLON) || tokenType.equals(PropsTypes.EQUALS)) {
            return SEPARATOR_KEYS;
        }
        if (tokenType.equals(PropsTypes.GROUP_PART)) {
            return DEPENDENCY_GROUP_KEYS;
        }
        if (tokenType.equals(PropsTypes.NAME_KEY)) {
            return DEPENDENCY_NAME_KEYS;
        }
        if (tokenType.equals(PropsTypes.VERSION)) {
            return DEPENDENCY_VERSION_KEYS;
        }
        if (tokenType.equals(PropsTypes.COMMENT)) {
            return COMMENT_KEYS;
        }
