...
public abstract class DependencyGroup {
    ...
    public static DependencyGroup groupFromParameters(CompletionParameters parameters) {
        ...
        if (currentElement == null && position.getNode().getElementType() == PropsTypes.NAME_KEY) {
            currentElement = position.getParent();
        }

        List<String> newParts = new ArrayList<>();
        while (currentElement != null) {
            if (currentElement.getNode().getElementType() == PropsTypes.GROUP_PART) {
                newParts.add(0, currentElement.getText());
            } else if (currentElement.getNode().getElementType() == PropsTypes.DEPENDENCY_GROUP) {
