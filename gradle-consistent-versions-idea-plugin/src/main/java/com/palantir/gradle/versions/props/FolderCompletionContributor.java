...
public class FolderCompletionContributor extends CompletionContributor {
    ...
    public FolderCompletionContributor() {
        cacheCompletion(PropsTypes.GROUP_PART);
        cacheCompletion(PropsTypes.NAME_KEY);
        remoteCompletion(PropsTypes.GROUP_PART);
        remoteCompletion(PropsTypes.NAME_KEY);
    }
        ...
        extend(CompletionType.BASIC, PlatformPatterns.psiElement(elementType), new CompletionProvider<>() {
            ...
            protected void addCompletions(
                ...
                GradleCacheExplorer.getInstance()
                        .getCompletions(
                                RepositoryLoader.loadRepositories(project), group, elementType == PropsTypes.NAME_KEY)
                        .stream()
