...
import com.palantir.gradle.versions.props.psi.PropsDependencyVersion;
import com.palantir.gradle.versions.props.psi.PropsProperty;
...
public class VersionCompletionContributor extends CompletionContributor {
    ...
    VersionCompletionContributor() {
        extend(CompletionType.BASIC, PlatformPatterns.psiElement(PropsTypes.VERSION), new CompletionProvider<>() {
            @Override
            public void addCompletions(
                    CompletionParameters parameters, ProcessingContext context, CompletionResultSet resultSet) {

                PropsDependencyVersion versionElement = ReadAction.compute(
                        () -> (PropsDependencyVersion) parameters.getPosition().getParent());

                PropsProperty property = ReadAction.compute(() -> findParentProperty(versionElement));
    ...
    private PropsProperty findParentProperty(PropsDependencyVersion versionElement) {
        return versionElement == null ? null : PsiTreeUtil.getParentOfType(versionElement, PropsProperty.class);
    }
